...
Recent project behavior now documented here and in `docs/*.md`:

- Tech+ Questions mode includes a dedicated **Study plans** group with:
  - Voucher custom study plan row
  - **Weighted topic cram (full read)** lesson (`assets/learn-hub-tech-weighted-cram.js`)
- Voucher study plan now has:
  - **Reset study tracking** (voucher-only metrics)
  - clearer topic-tracking explainer text
  - **Practice 15 from top weak topics** wording
- Voucher scoring includes a transparent **practice 100–900 estimate** derived from configured domain/bucket weights in `assets/learn-hub-voucher01-plan.js` (official exam scoring remains proprietary).
- Lesson text bundles had additional OCR/spacing cleanup passes across chapter and generated lesson content.
...
| `assets/learn-hub-voucher01-plan.js` | Tech+ voucher study-plan mapping and metadata: question-to-domain mapping, study-plan hints, exam-bucket weighting configuration, and helpers consumed by `learn-hub-app.js` for drill targeting and weighted score estimation. |
| `assets/learn-hub-tech-weighted-cram.js` | Standalone weighted cram reading lesson content shown in Tech+ **Questions → Study plans**. |
...
2. **Reading content** — behavior depends on track and lesson:
  ...
  - **Tech+ Questions mode** (sidebar Chapters/Objectives/Questions switch): when set to **Questions**, the sidebar shows imported GimKit quiz lessons (`tech-gimkit-*`) plus Study-plan entries. Quiz rows stay quiz-focused; the teach/notes pane is restored for study-plan reading rows such as weighted cram.
...
| `scripts/build-techplus-from-markdown.mjs` | Reads **`chapters/*.md`**, preprocesses PDF-style line breaks and objective bullets, runs **marked** (GFM **without** hard line breaks — avoids a wall of `<br>` tags), applies **`polishTechplusHtml`**, writes the Tech+ chapter loader and chunk bundles (`learn-hub-techplus-md.js` + `learn-hub-techplus-md-chNN.js`). If chapter lesson IDs are missing from the Tech+ course, it **prepends** `tech-sg-01` … `tech-sg-12` to `learn-hub-courses.js`. **Re-run after you edit any bundled chapter `.md` file.** |
