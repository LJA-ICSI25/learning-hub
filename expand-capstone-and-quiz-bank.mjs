 ...
 * Run once: node expand-capstone-and-quiz-bank.mjs
 */
import fs from "fs";
...
function loadCourses() {
  const t = fs.readFileSync(path.join(dir, "learn-hub-courses.js"), "utf8");
  return JSON.parse(t.replace(/^[\s\S]*?=\s*/, "").replace(/;\s*$/, ""));
}

function loadDepth() {
  const t = fs.readFileSync(path.join(dir, "learn-hub-depth.js"), "utf8");
  return JSON.parse(t.replace(/^[\s\S]*?=\s*/, "").replace(/;\s*$/, ""));
}
