 ...
 * Run: node expand-tracks-more.mjs
 */
import fs from "fs";
...
function loadCourses() {
  return JSON.parse(
    fs.readFileSync(path.join(dir, "learn-hub-courses.js"), "utf8").replace(/^[\s\S]*?=\s*/, "").replace(/;\s*$/, "")
  );
}

function loadDepth() {
  return JSON.parse(
    fs.readFileSync(path.join(dir, "learn-hub-depth.js"), "utf8").replace(/^[\s\S]*?=\s*/, "").replace(/;\s*$/, "")
  );
}

function saveCourses(courses) {
  fs.writeFileSync(
    path.join(dir, "learn-hub-courses.js"),
...
function saveDepth(D) {
  fs.writeFileSync(
    path.join(dir, "learn-hub-depth.js"),
